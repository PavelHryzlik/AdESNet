...
#include <exception>
#include "AdESNet.CLRFramework.h"

namespace AdESNetCLRFramework
{
	...
	AdESNetCLR::AdESResult^ AdESWrapper::CAdESSign(
		array<Byte>^ dataToSign, 
		AdESNetCLR::SignatureContext^ signatureContext)
	{
		...
		AdESNetCLR::AdESResult^ result = gcnew AdESNetCLR::AdESResult();
		result->Status = res;
		result->Error = "";
	...
	AdESNetCLR::AdESResult^ AdESWrapper::PAdESSign(
		array<Byte>^ dataToSign, 
		AdESNetCLR::PAdESSignatureContext^ signatureContext)
	{
		if (signatureContext == nullptr ||
			signatureContext->SigningCertificate == nullptr ||
			signatureContext->AdESAttributes == nullptr)
	...
	AdESNetCLR::AdESResult^ AdESWrapper::XAdESSign(
		array<Byte>^ dataToSign, 
		AdESNetCLR::XAdESSignatureContext^ signatureContext)
	{
		if (signatureContext == nullptr ||
			signatureContext->SigningCertificate == nullptr ||
			signatureContext->AdESAttributes == nullptr ||
